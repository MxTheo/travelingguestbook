...
class TestHome:
    ...
    def test_if_words_are_shown(self, client):
        """Test that when there are 6 words, that there are 3 shown on the homepage"""
        for i in range(7):
            WordFactory(word=f"word{i}")
        response = client.get(reverse('home'))
        for i in range(3):
            assert f"word{i}" not in response.text
        for i in range(4, 7):
            assert f"word{i}" in response.text
