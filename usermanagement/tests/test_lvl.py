...
from travelingguestbook.factories import StreetActivityFactory, WordFactory
...
class TestLvl:
    ...
    def test_that_word_is_added(self, auto_login_user):
        """Given a user of 0xp who describes 1 word, test if the user gets 50xp"""
        ...
        url = reverse("create-word", args=[activity.id])

        client.post(url, data_word, follow=True)

        profile = Profile.objects.get(user=user)
        assert profile.xp == 50

    def test_from_lvl_1_to_2(self, auto_login_user):
        """Given a user of lvl 1 who describes 2 words,
...
class TestProgress:
    ...
    def test_progress_in_between(self, auto_login_user):
        """Given a user of lvl 1 who describes 1 word,
    ...
    def test_progress_lvl_up(self, auto_login_user):
        """Given a user of lvl 1 describing 2 words,
        tests if they lvl up"""
        client, user = auto_login_user()
        activity = StreetActivityFactory()
        word = WordFactory()
